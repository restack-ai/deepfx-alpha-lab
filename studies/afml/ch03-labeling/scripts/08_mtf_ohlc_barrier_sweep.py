...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Run M5 event + M1 OHLC execution-aware triple-barrier sweep.")
    ...
    parser.add_argument("--run-name", default="ohlc_sl_first")
    parser.add_argument(
        "--output-dir",
        type=Path,
...
def summarize(labels: pd.DataFrame, metadata: dict[str, object], pt: float, sl: float, num_days: float) -> dict[str, object]:
    ...
    neutral = count_value(labels["bin"], 0) if labeled_events else 0
    ...
    ambiguous_count = int(labels["ambiguous"].sum()) if labeled_events and "ambiguous" in labels else 0
    holding_minutes = (
    ...
    return {
        **metadata,
        ...
        "ambiguous_rate": ratio(ambiguous_count, labeled_events),
...
def main() -> None:
    ...
    for num_days in num_days_grid:
        t1 = add_vertical_barrier(t_events, path_bars["close"], num_days=num_days)
    ...
    stem = (
        f"mtf_ohlc_barrier_sweep_{args.symbol.lower()}_{args.event_timeframe.lower()}_event_"
